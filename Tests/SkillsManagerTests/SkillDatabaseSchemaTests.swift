...
struct SkillDatabaseSchemaTests {
    ...
    func createsAndReopensCurrentSchema() throws {
        ...
        do {
            ...
            #expect(try connection.querySingleInt("PRAGMA user_version") == 6)
            #expect(try connection.userTableNames() == SkillSchemaV6.tableNames)
            ...
            #expect(strictCount == 8)
        }

        let reopened = try SkillSchemaMigrator.open(at: location.database)
        #expect(try reopened.querySingleInt("PRAGMA user_version") == 6)
    ...
    func handlesAConcurrentV0Migration() throws {
        ...
        #expect(try first.querySingleInt("PRAGMA user_version") == 6)
        #expect(try second.querySingleInt("PRAGMA user_version") == 6)
        #expect(try second.userTableNames() == SkillSchemaV6.tableNames)
    }
    ...
    func rejectsInvalidSchemaStates() throws {
        ...
        try futureConnection.execute("PRAGMA user_version = 7")
        #expect(throws: SQLiteStoreError.self) {
