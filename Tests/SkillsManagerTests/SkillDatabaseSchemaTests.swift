...
struct SkillDatabaseSchemaTests {
        ...
        do {
            ...
            let strictCount = try connection.querySingleInt(
                "SELECT count(*) FROM pragma_table_list "
                    ...
                    + "'provider_provenance','managed_publish_states') "
                    + "AND strict = 1"
            )
            #expect(strictCount == 13)
        }

        let reopened = try SkillSchemaMigrator.open(at: location.database)
        #expect(try reopened.querySingleInt("PRAGMA user_version") == 12)
    ...
    func handlesAConcurrentV0Migration() throws {
        ...
        #expect(try first.querySingleInt("PRAGMA user_version") == 12)
        #expect(try second.querySingleInt("PRAGMA user_version") == 12)
        #expect(try second.userTableNames() == SkillSchemaV12.tableNames)
    }
    ...
    func rejectsInvalidSchemaStates() throws {
        ...
        try futureConnection.execute(
            "PRAGMA user_version = \(SkillSchemaV12.version + 1)"
        )
        #expect(throws: SQLiteStoreError.self) {
