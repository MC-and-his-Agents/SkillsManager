...
struct SkillDatabaseSchemaTests {
    ...
    func createsAndReopensCurrentSchema() throws {
        ...
        do {
            ...
            #expect(try connection.querySingleInt("PRAGMA user_version") == 8)
            #expect(try connection.userTableNames() == SkillSchemaV8.tableNames)
            ...
            let strictCount = try connection.querySingleInt(
                "SELECT count(*) FROM pragma_table_list "
                    ...
                    + "'distribution_bindings','distribution_operations',"
                    + "'distribution_link_ownership','distribution_configurations') "
                    + "AND strict = 1"
            )
            #expect(strictCount == 11)
        }

        let reopened = try SkillSchemaMigrator.open(at: location.database)
        #expect(try reopened.querySingleInt("PRAGMA user_version") == 8)
    ...
    func handlesAConcurrentV0Migration() throws {
        ...
        #expect(try first.querySingleInt("PRAGMA user_version") == 8)
        #expect(try second.querySingleInt("PRAGMA user_version") == 8)
        #expect(try second.userTableNames() == SkillSchemaV8.tableNames)
    }
    ...
    func rejectsInvalidSchemaStates() throws {
        ...
        try futureConnection.execute("PRAGMA user_version = 9")
        #expect(throws: SQLiteStoreError.self) {
